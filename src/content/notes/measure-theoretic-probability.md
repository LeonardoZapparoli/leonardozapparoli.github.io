...
code: Prob
description: "From elementary to measure-theoretic probability: conditioning as reweighting, the Radon–Nikodym theorem, conditional expectation given a σ-algebra, conditional laws and disintegration, Bayes' theorem, and the Borel–Kolmogorov paradox."
date: 2026-09-12
---

These notes are a bridge between two ways of doing probability.

*Elementary probability*, as taught in a first course, defines conditional probability by division, $\P(A \mid B) = \P(A \cap B)/\P(B)$; conditional expectation given a random variable $Y$ by averaging over the level sets $\{Y = y\}$; a density as a function one integrates to get probabilities; and Bayes' theorem as a ratio of sums or integrals. These definitions are correct, but each carries hypotheses that are easy to forget: the conditioning event must have positive probability, $Y$ must take countably many values, the density must exist. Outside those hypotheses the definitions do not merely become awkward — they have no meaning. One cannot divide by $\P(Y = y) = 0$, and one cannot glue uncountably many conditional expectations into a random variable by countable additivity. Yet the objects one wants to talk about — the conditional density of $Y$ given $X = x$, the posterior distribution of a continuous parameter given continuous data — live precisely outside the hypotheses.

*Measure-theoretic probability*, in the form given by Kolmogorov in 1933, replaces these definitions with ones phrased in terms of measures, measurable functions, and $\sigma$-algebras: a probability is a measure, a random variable is a measurable function, a state of information is a $\sigma$-algebra, and conditioning is a Radon–Nikodym derivative. The gain is not rigor for its own sake. A single definition of conditional expectation covers the discrete, continuous, and mixed cases at once; conditioning on an event of probability zero acquires a meaning; and the resulting objects behave well under limits, which is what makes the theory of stochastic processes, martingales, and statistical inference possible.

Every elementary result reappears as the special case of a general one, and part of the purpose of these notes is to make the correspondence explicit. Elementary definitions and theorems are stated with the hypotheses under which they hold, and the general statement subsuming each is proved. The main correspondences are the following.

| Elementary | Hypothesis | General |
|---|---|---|
| $\P(A \mid B) = \P(A \cap B)/\P(B)$ | $\P(B) > 0$ | $\P(A \mid \cH) = \E[\ind{A} \mid \cH]$ for a $\sigma$-algebra $\cH$ |
| $\E[X \mid Y = y] = \E[X \ind{\{Y = y\}}]/\P(Y = y)$ | $Y$ countably valued | $\E[X \mid Y = y] = \frac{d\mu_X}{d\P_Y}(y)$, $\mu_X(B) = \E[X \ind{\{Y \in B\}}]$ |
| pdf $f_X$ with $\P(X \in B) = \int_B f_X$ | $\P_X \ll$ Lebesgue | $f_X = d\P_X / d\mathrm{Leb}$, any dominating measure allowed |
| $\E[Y \mid X = x] = \int y\, f(x,y)\dd y / f_X(x)$ | joint density exists | mean of the conditional law $\kappa(x, \cdot)$ |
| conditional pdf $f_{Y \mid X}(y \mid x) = f(x,y)/f_X(x)$ | joint density exists | conditional law: a kernel $\kappa(x, \cdot)$ disintegrating the joint law |
| Bayes: $\P(B_j \mid A) \propto \P(A \mid B_j)\P(B_j)$ | countable partition, or densities | posterior kernel; $d\Pi(x,\cdot)/d\pi = p(\theta, x)/m(x)$ |

The aim throughout is to make clear why the general definitions are what they are, not only to state them. Every result used is proved or explicitly cited; the only nontrivial imports are the Lebesgue differentiation theorem (used for interpretation, not in any proof) and the Borel isomorphism theorem (used to pass from $\R$ to general standard Borel spaces).

The prerequisite is a first course in measure theory: $\sigma$-algebras, measurable functions, the Lebesgue integral, monotone and dominated convergence, $L^p$ spaces. Fubini's theorem and the change of variables formula are recalled in the preliminaries.

One idea organizes the material. Conditioning on an event of positive probability is division: restrict the measure to the event and renormalize. Conditioning on an event of probability zero cannot be division, because numerator and denominator both vanish. What survives is a ratio of rates: the rate at which some quantity accumulates near the event, relative to the rate at which probability accumulates there. The Radon–Nikodym theorem is the tool that makes ratios of rates rigorous, and it is behind every construction in these notes.

## Conditioning on Events

### Where the elementary definition comes from

Let $(\Omega, \cF, \P)$ be a probability space and let $B \in \cF$ with $\P(B) > 0$. We are told that $B$ has occurred and we want to replace $\P$ by a probability measure $\P_B$ on $(\Omega, \cF)$ that reflects this information. Two requirements are forced by the meaning of the situation.

(C1) *Outcomes outside $B$ are ruled out:* $\P_B(B) = 1$.

(C2) *Relative weights inside $B$ are unchanged:* there is a constant $c > 0$ such that $\P_B(A) = c\,\P(A)$ for every event $A \subseteq B$.

Condition (C2) is the substantive one. It says that for $A_1, A_2 \subseteq B$ the odds $\P_B(A_1) : \P_B(A_2)$ equal the odds $\P(A_1) : \P(A_2)$. The information "$B$ occurred" says nothing about *where inside $B$* the outcome lies; if, before learning $B$, one part of $B$ was twice as likely as another, learning only that the outcome is somewhere in $B$ gives no reason to change that. Conditioning may delete mass outside $B$; it may not rebalance mass within $B$.

:::proposition{#elementary-conditioning title="Elementary Conditioning Is Forced"}
The unique probability measure $\P_B$ satisfying (C1) and (C2) is
...
:::proof
Taking $A = B$ in (C2) and using (C1) gives $c = 1/\P(B)$. For general $A$, additivity and (C1) give $\P_B(A) = \P_B(A \cap B) + \P_B(A \cap B^c) = \P(A \cap B)/\P(B) + 0$. The displayed formula is a probability measure and satisfies (C1), (C2).
:::

We write $\P(A \mid B)$ for $\P_B(A)$. The formula is usually presented as a definition; the proposition shows it is not a choice.

The result can be restated in the form that will recur throughout these notes: $\P_B$ is $\P$ multiplied by the function $\ind{B}/\P(B)$,

$$
\P_B(A) = \int_A \frac{\ind{B}}{\P(B)} \dd\P .
$$

Conditioning is reweighting. The conditional measure has a density with respect to the original one, here $\ind{B}/\P(B)$, which deletes mass outside $B$ and rescales the mass inside $B$ uniformly. The section on the Radon–Nikodym theorem studies densities in general, and conditioning on events of probability zero will amount to finding the right density when division is no longer available.
...
Both hypotheses are used: countability of the partition for countable additivity, positivity of the $\P(B_i)$ so that the conditional probabilities exist. The subsection on Bayes' theorem in the final section removes both.
...
In odds form, for two hypotheses,
...
the data act on prior odds through the likelihood ratio alone. This is reweighting once more; the posterior is the prior multiplied by $j \mapsto \P(A \mid B_j)$ and normalized.
...
If $\P(B) > 0$, independence of $A$ and $B$ says $\P(A \mid B) = \P(A)$: learning $B$ does not change the probability of $A$. To check independence of $\sigma$-algebras it suffices to check the product formula on $\pi$-systems generating them (Dynkin's $\pi$–$\lambda$ theorem), which is why independence of real random variables can be verified on the events $\{X_i \le x_i\}$. Conditional independence is the more important notion in statistics: it is the structure of Markov chains, of graphical models, and of the assumption that observations are independent given a parameter. Its general form, without the hypothesis $\P(C) > 0$, is in the subsection on conditional independence in the final section.
...
Two things about this proposition should be noticed. First, properties (i) and (ii) characterize $\E[X \mid \cP]$ by (iii), and both make sense with an arbitrary sub-$\sigma$-algebra in place of $\sigma(\cP)$; that is the definition adopted in the section on conditional expectation. Second, the tower property says that coarsening the partition flattens the function: $\E[X \mid \cP']$ is obtained from $\E[X \mid \cP]$ by averaging again, and the trivial partition $\{\Omega\}$ gives the constant $\E X$. The local-averaging figure in the section on conditional expectation shows the construction for two nested partitions of $[0,1]$.
...
The resolution has two parts. First, the integral identity (ii) of [[#elementary-ce-properties]] survives when the cells are null, because it refers only to sets in the $\sigma$-algebra, and a set in the $\sigma$-algebra either is null or has positive probability; this leads to the definition of conditional expectation given a $\sigma$-algebra. Second, the cell averages themselves are recovered as limits of averages over thin slabs $\{x \le X < x + \eps\}$, which are not null; this is the Radon–Nikodym derivative computed pointwise, and it turns the elementary formula into a theorem.
...
A function $X : \Omega \to \R$ is **$\cH$-measurable** if $\{X \in B\} \in \cH$ for all $B \in \cB(\R)$; equivalently $\sigma(X) \subseteq \cH$, where $\sigma(X) = \{X^{-1}(B) : B \in \cB(\R)\}$.
:::

In the information reading, $X$ is $\cH$-measurable when every question "is $X \in B$?" can be answered from $\cH$: the value of $X$ is determined by the information in $\cH$. It suffices to check $\{X \le q\} \in \cH$ for rational $q$, since $\{B : X^{-1}(B) \in \cH\}$ is a $\sigma$-algebra containing the half-lines, which generate $\cB(\R)$.
...
No probabilistic quantity distinguishes a.s.-equal random variables, so the class is the most that probabilistic data could ever determine. Knowing the *law* of $X$ is much less than knowing $X$: on $[0,1]$ with Lebesgue measure, $X(\omega) = \omega$ and $X'(\omega) = 1 - \omega$ have the same law and are almost nowhere equal. Knowing $X$ means knowing the events $\{X \in B\}$ as subsets of $\Omega$ up to null sets, not merely their probabilities.

### The uniqueness lemma

The following lemma is used repeatedly. It is the uniqueness half of the Radon–Nikodym theorem, the uniqueness half of conditional expectation, and the tool for passing from "all integrals agree" to "the functions agree."
...
:::proof
If $X = g(Y)$ then $X^{-1}(B) = Y^{-1}(g^{-1}(B)) \in \sigma(Y)$.
...
The lemma says that "$X$ uses only the information in $Y$" and "$X$ is a function of $Y$" are the same statement. We use it to define $\E[X \mid Y = y]$, and later to pass from objects indexed by $\omega$ to objects indexed by the value of a random variable.

### The standard machine

Many identities below are linear in a test function and are proved by the same escalation: verify the identity for indicators of measurable sets, extend to simple functions by linearity, to nonnegative functions by monotone convergence, and to general functions by writing $Z = Z^+ - Z^-$. We call this the **standard machine**. When the identity is between two measures rather than two integrals, Dynkin's $\pi$–$\lambda$ theorem plays the same role: two probability measures agreeing on a $\pi$-system agree on the $\sigma$-algebra it generates.
...
Let $X : \Omega \to S$ be measurable. The **law** of $X$ is $\P_X = \P \circ X^{-1}$, and for measurable $g \ge 0$ (or $g(X) \in L^1$),
...
Two readings of $f$ go together.

- *Global:* $f$ is the unique function whose integrals over sets reproduce $\nu$. [[#uniqueness-lemma]] says the map $f \mapsto f\,d\mu$ is injective; the theorem says its image is exactly the measures absolutely continuous with respect to $\mu$.
- *Local:* when the measures live on $\R^n$ and $\mu$ is Lebesgue measure, $f(x) = \lim_{\eps \downarrow 0} \nu(B_\eps(x))/\mu(B_\eps(x))$ for $\mu$-a.e. $x$, by the Lebesgue differentiation theorem ([[#lebesgue-differentiation]]). The derivative notation is literal: $f$ is a limit of ratios of masses of shrinking sets, finite although both masses tend to zero.

:::remark{#densities-elementary title="Densities in Elementary Probability"}
In elementary probability a random variable $X$ "has a density $f_X$" if $\P(X \in B) = \int_B f_X(x) \dd x$ for all Borel $B$. In the present language this says exactly that $\P_X \ll \mathrm{Leb}$ with $f_X = d\P_X/d\mathrm{Leb}$: the probability density function of elementary probability is the Radon–Nikodym derivative of the law of $X$ with respect to Lebesgue measure. Likewise a probability mass function is $d\P_X/d\#$, the derivative with respect to counting measure, and a joint density on $\R^2$ is $d\P_{(X,Y)}/d\mathrm{Leb}^2$. The theorem then tells us exactly which random variables have densities — those whose law gives probability zero to every Lebesgue-null set, and no others — and the elementary heuristic "$f_X(x)\,dx$ is the probability of landing in $[x, x + dx]$" is the local reading of the derivative above. Nothing in the elementary theory is lost; what is gained is that the same word "density" now applies with respect to any reference measure, which is what one needs for mixed distributions, for distributions on non-Euclidean spaces, and for likelihood ratios.
:::

### Two facts about Hilbert spaces

The proof below, due to von Neumann, uses two facts about a real Hilbert space $H$, proved here for completeness.

:::lemma{#closest-point title="Closest Point in a Closed Convex Set"}
Let $C \subseteq H$ be nonempty, closed and convex, and $x \in H$. There is a unique $y^* \in C$ with $\norm{x - y^*} = d \coloneqq \inf_{y \in C} \norm{x - y}$.
:::

:::proof
Let $y_n \in C$ with $\norm{x - y_n} \to d$. The parallelogram law applied to $y_n - x$ and $y_m - x$ gives

$$
\norm{y_n - y_m}^2
= 2\norm{y_n - x}^2 + 2\norm{y_m - x}^2 - 4\,\Big\lVert \tfrac{y_n + y_m}{2} - x \Big\rVert^2
\le 2\norm{y_n - x}^2 + 2\norm{y_m - x}^2 - 4d^2,
$$

since $\frac{y_n + y_m}{2} \in C$. The right side tends to $0$, so $(y_n)$ is Cauchy and converges to some $y^* \in C$ with $\norm{x - y^*} = d$. The same identity applied to two minimizers shows they coincide.
:::

:::theorem{#riesz title="Riesz Representation"}
Every bounded linear functional $\ell : H \to \R$ has the form $\ell(x) = \ip{x}{z}$ for a unique $z \in H$.
:::

:::proof
If $\ell = 0$ take $z = 0$. Otherwise $K = \ker \ell$ is a proper closed subspace. Pick $x_0 \notin K$, let $k^*$ be its closest point in $K$, and put $u = x_0 - k^* \ne 0$. For $k \in K$ and $t \in \R$, $\norm{u - tk}^2 \ge \norm{u}^2$ expands to $-2t\ip{u}{k} + t^2\norm{k}^2 \ge 0$ for all $t$, forcing $\ip{u}{k} = 0$; so $u \perp K$. For any $x$ the vector $\ell(x)u - \ell(u)x$ lies in $K$, so $\ell(x)\norm{u}^2 = \ell(u)\ip{x}{u}$, i.e. $\ell(x) = \ip{x}{z}$ with $z = \ell(u)u/\norm{u}^2$. If $\ip{x}{z} = \ip{x}{z'}$ for all $x$, take $x = z - z'$.
:::

### Proof of the theorem

:::proof{title="Proof of the Radon–Nikodym theorem for finite measures"}
Let $\rho = \mu + \nu$ and $H = L^2(\rho)$.

*Step 1.* The functional $\ell(g) = \int g \dd\nu$ on $L^2(\rho)$ is linear and bounded: $\abs{\ell(g)} \le \int \abs{g} \dd\rho \le \norm{g}_{L^2(\rho)}\,\rho(\Omega)^{1/2}$. By [[#riesz]] there is $h \in L^2(\rho)$ with

$$
\int g \dd\nu = \int g\,h \dd\rho \qquad \text{for all } g \in L^2(\rho) .
$$
{#von-neumann-identity}

*Step 2: $0 \le h \le 1$ $\rho$-a.e.* With $A_n = \{h \le -1/n\}$ and $g = \ind{A_n}$, [[#von-neumann-identity]] gives $0 \le \nu(A_n) = \int_{A_n} h \dd\rho \le -\rho(A_n)/n$, so $\rho(A_n) = 0$. With $A_n = \{h \ge 1 + 1/n\}$ it gives $\nu(A_n) = \int_{A_n} h \dd\rho \ge (1 + 1/n)\rho(A_n) \ge (1 + 1/n)\nu(A_n)$, so $\nu(A_n) = 0$, and then $0 = \int_{A_n} h \dd\rho \ge (1 + 1/n)\rho(A_n)$ gives $\rho(A_n) = 0$. Take unions over $n$ and modify $h$ on a $\rho$-null set so that $0 \le h \le 1$ everywhere.

*Step 3.* Since $\rho = \mu + \nu$, [[#von-neumann-identity]] rearranges to

$$
\int g\,(1 - h) \dd\nu = \int g\,h \dd\mu
\qquad \text{for all } g \in L^2(\rho) .
$$
{#key-identity}

*Step 4.* Let $B = \{h = 1\}$. With $g = \ind{B}$ in [[#key-identity]] the left side is $0$, so $\mu(B) = \int_B h \dd\mu = 0$. Set $\nu_{\mathrm{s}}(A) = \nu(A \cap B)$ and $\nu_{\mathrm{ac}}(A) = \nu(A \cap B^c)$; then $\nu = \nu_{\mathrm{s}} + \nu_{\mathrm{ac}}$ and $\nu_{\mathrm{s}} \perp \mu$.

*Step 5.* Fix $A \in \cG$ and put $g = \ind{A}(1 + h + \cdots + h^n)$ in [[#key-identity]]; since $(1-h)(1 + h + \cdots + h^n) = 1 - h^{n+1}$,

$$
\int_A (1 - h^{n+1}) \dd\nu = \int_A h\,(1 + h + \cdots + h^n) \dd\mu .
$$

Let $n \to \infty$. On $B^c$, $1 - h^{n+1} \uparrow 1$; on $B$ the left integrand is $0$; so by monotone convergence the left side tends to $\nu(A \cap B^c) = \nu_{\mathrm{ac}}(A)$. The right integrands increase to $h/(1-h)$ on $B^c$ and to $+\infty$ on the $\mu$-null set $B$, so the right side tends to $\int_A f \dd\mu$ with

$$
f = \frac{h}{1-h}\,\ind{B^c}
$$

(the value of $f$ on $B$ is immaterial). Thus $\nu_{\mathrm{ac}}(A) = \int_A f \dd\mu$ for all $A$, which shows $\nu_{\mathrm{ac}} \ll \mu$, and $\int f \dd\mu = \nu(B^c) < \infty$. If $\nu \ll \mu$ then $\mu(B) = 0$ forces $\nu(B) = 0$, so $\nu_{\mathrm{s}} = 0$.

*Uniqueness.* Two densities of $\nu_{\mathrm{ac}}$ have equal integrals over all $A \in \cG$ and agree a.e. by [[#uniqueness-lemma]]. If $\nu_{\mathrm{ac}} + \nu_{\mathrm{s}} = \nu'_{\mathrm{ac}} + \nu'_{\mathrm{s}}$ with singular parts carried by $\mu$-null sets $B, B'$, then $E = B \cup B'$ is $\mu$-null, both absolutely continuous parts vanish on subsets of $E$, and $\nu_{\mathrm{s}}(A) = \nu(A \cap E) = \nu'_{\mathrm{s}}(A)$ for all $A$; subtracting gives $\nu_{\mathrm{ac}} = \nu'_{\mathrm{ac}}$.
:::

:::proof{title="Proof for $\sigma$-finite measures"}
Let $(\Omega_k)$ be a countable measurable partition of $\Omega$ into sets of finite $\mu$-measure and finite $\nu$-measure (the common refinement of exhaustions for $\mu$ and for $\nu$). Apply the finite case on each $\Omega_k$, obtaining densities $f_k$ and singular carriers $B_k \subseteq \Omega_k$, and put $f = \sum_k f_k \ind{\Omega_k}$ and $B = \bigcup_k B_k$. Countable additivity assembles the decomposition, and uniqueness is as before.
:::

The representer $h$ produced by the Riesz theorem is the density of $\nu$ with respect to $\mu + \nu$, which is not the quantity asked for. Steps 3–5 change the denominator, $\frac{d\nu}{d\mu} = \frac{h}{1-h}$, and the set $\{h = 1\}$, on which all local mass is $\nu$-mass, turns out to carry the singular part. The Lebesgue decomposition comes out of the same computation rather than needing a separate one.

### Likelihood ratios and change of measure

In statistics the Radon–Nikodym derivative appears as the *likelihood ratio*. Let $P$ and $Q$ be two candidate distributions for the same data on a sample space $(S, \cS)$, both absolutely continuous with respect to a $\sigma$-finite reference measure $\lambda$, with densities $p = dP/d\lambda$ and $q = dQ/d\lambda$.
...
In words: a likelihood ratio is a local correction factor. At each sample point $x$ it says by how much to multiply $P$-mass near $x$ to obtain $Q$-mass near $x$, and integrating $P$ against it reproduces $Q$. That is precisely what a Radon–Nikodym derivative is; the likelihood ratio of elementary statistics *is* $dQ/dP$, computed as $q/p$ when both models are described by densities. The same mechanism underlies exponential tilting, Girsanov's theorem, and the Kullback–Leibler divergence $D(Q \Vert P) = \E_Q[\log \frac{dQ}{dP}]$, which is defined exactly when $Q \ll P$ (see [[InfoTheory]]).

## Conditional Expectation

### What an observer with partial information can measure

Fix a sub-$\sigma$-algebra $\cH \subseteq \cF$ and an integrable random variable $X$. Consider an observer who knows, for every $A \in \cH$, whether $A$ occurred, and knows nothing else. What can this observer say about $X$?

They cannot evaluate $X(\omega)$: that requires knowing which point $\omega$ occurred, which is more information than $\cH$ contains unless $X$ happens to be $\cH$-measurable. What they can do is form weighted averages of $X$ using weights they are able to compute. A weight is a bounded $\cH$-measurable random variable $Z$: a bet size chosen on the basis of $\cH$-information, a filter selecting some outcomes and not others, the indicator of a subpopulation the observer can identify. For each such $Z$ the number $\E[Z X]$ is accessible to them, in the way that any expectation is accessible — by averaging over many independent repetitions of the experiment, weighting each by the $Z$ computed at the time. We call a bounded $\cH$-measurable $Z$ an **$\cH$-test**, and the numbers $\E[ZX]$ the **$\cH$-observable averages** of $X$.
...
So two integrable random variables are indistinguishable to the observer — they agree in every average the observer can compute — exactly when they have the same integral over every set in $\cH$.

Now ask what the best description of $X$ available to this observer is. It should be a random variable they can actually evaluate, which means an $\cH$-measurable one. And it should not be distinguishable from $X$ by anything they can measure, which by the proposition means it has the same integral as $X$ over every $\cH$-set. Nothing else can be asked: any two $\cH$-measurable candidates satisfying these requirements agree in every $\cH$-observable average, and we will see that this forces them to agree a.s. These two requirements are the definition.
...
Any two such $Y$ agree a.s. ([[#ce-existence-uniqueness]]); the class is denoted $\E[X \mid \cH]$. For $B \in \cF$ put $\P(B \mid \cH) = \E[\ind{B} \mid \cH]$. Conditioning on a random variable is defined below as conditioning on the $\sigma$-algebra it generates.
:::
...
Both conditions are met by the construction for a reason. Partial averaging is the defining property of a density. The $\cH$-measurability comes from applying Radon–Nikodym on the *smaller* measurable space $(\Omega, \cH)$, whose measurable functions are exactly the $\cH$-measurable ones. In one line,

$$
\E[X \mid \cH] = \frac{d\,(X\,d\P)|_\cH}{d\,\P|_\cH} :
$$

restrict the measure $X\,d\P$ and the measure $\P$ to $\cH$, forgetting every distinction $\cH$ cannot make, and take the density of one with respect to the other at that resolution.
...
Three readings of the defining condition [[#test-identity]] are standard and worth having available.

- *Bias.* If $\int_A Y \ne \int_A X$ for some $A \in \cH$, an observer with information $\cH$ can identify a subpopulation on which $Y$ is systematically wrong about $X$. Partial averaging says there is none.
- *Fair pricing.* If $Y$ is the price quoted for the payoff $X$, a trader allowed to condition on $\cH$ makes expected profit by buying on sets $A$ with $\int_A (X - Y) > 0$ and selling where the integral is negative. Partial averaging says no such strategy exists. For a filtration $(\cF_t)$ this is the martingale condition: an adapted integrable $M$ is a martingale iff $\E[Z_t (M_{t+1} - M_t)] = 0$ for every bounded $\cF_t$-measurable $Z_t$.
- *Geometry.* In $L^2$, [[#test-identity]] says $X - Y$ is orthogonal to the subspace of $\cH$-measurable functions (see the $L^2$ subsection below).

The definition involves no loss function. The $L^2$ picture is a consequence valid under a stronger hypothesis; the definition itself lives in $L^1$, where there is no inner product, and its content is the matching of all $\cH$-observable averages.
...
The **conditional expectation of $X$ given $Y$** is the conditional expectation of $X$ given the $\sigma$-algebra generated by $Y$:
...
The section on conditional expectation from a joint density computes $g$ explicitly when $(X, Y)$ has a joint density.

### Properties

All identities hold a.s.; $X, X', X_n \in L^1$.
...
:::remark{#properties-in-words title="The Properties in Words"}
The tower, pull-out and independence properties can be understood, and in fact proved, directly from the picture of $\cH$-tests: $\E[X \mid \cH]$ is the $\cH$-measurable random variable that passes every $\cH$-test that $X$ passes, i.e. has the same $\cH$-observable averages as $X$. What follows is not a heuristic; each paragraph is the proof above said in words, and can be read as a proof on its own.

*Tower.* Let $\cG \subseteq \cH$, so the $\cG$-observer is coarser than the $\cH$-observer, and every $\cG$-test is an $\cH$-test. Write $V = \E[X \mid \cH]$ and $U = \E[V \mid \cG]$. Three facts:
...
*Independence.* Suppose $\sigma(X)$ is independent of $\cH$. The claim is that the constant $\E X$ is the $\cH$-summary of $X$. A constant is certainly $\cH$-measurable, so it remains to check the averages: for every $A \in \cH$,

$$
\int_A X \dd\P = \E[X \ind{A}] = \E X \cdot \P(A) = \int_A \E X \dd\P ,
$$

the middle equality by independence of $X$ and $\ind{A}$. So $\E X$ has the same $\cH$-observable averages as $X$, and by uniqueness $\E[X \mid \cH] = \E X$. In words: the average of $X$ over any subpopulation the observer can identify is the same as the overall average, so the observer's information is useless for predicting $X$. The converse fails: $\E[X \mid \cH] = \E X$ says only that the $\cH$-averages of $X$ are all equal (*mean independence*), not that the whole distribution of $X$ is unaffected by $\cH$.
:::

### Conditional variance and the law of total variance

For $X \in L^2$ the **conditional variance** is

$$
\Var(X \mid \cH) = \E\big[(X - \E[X \mid \cH])^2 \,\big|\, \cH\big]
= \E[X^2 \mid \cH] - \E[X \mid \cH]^2 ,
$$

the second form by expanding the square and using pull-out. Taking expectations,
...
The variance of $X$ is the average of the within-cell variances plus the variance of the cell averages. The second term is the part of the variance that $\cH$ accounts for; the first is what remains at resolution $\cH$. With $\cH = \sigma(\text{covariates})$ this is the decomposition of total variance into explained and residual variance.

### The L² picture and regression

:::theorem{#l2-projection title="Conditional Expectation as Orthogonal Projection"}
Let $X \in L^2$. Then $\E[X \mid \cH] \in L^2$ and for every $\cH$-measurable $W \in L^2$,
...
### How the averages determine a measurable function

If $X$ is $\cH$-measurable then $\E[X \mid \cH] = X$, so the data $(\int_A X)_{A \in \cH}$ determine $X$ a.s. by [[#uniqueness-lemma]]. It is useful to see how the averages encode the function, because the same mechanism governs conditioning on null events.

*Atomic case.* Call $a \in \cH$ an atom if $\P(a) > 0$ and every $\cH$-subset of $a$ has probability $0$ or $\P(a)$. An $\cH$-measurable $X$ is a.s. constant on each atom (otherwise some rational half-line $\{X \le q\}$ splits the atom into two $\cH$-sets of positive probability). Atoms are essentially disjoint with total mass at most $1$, so there are at most countably many, $a_1, a_2, \dots$. If they carry all the mass, $\P(\Omega \setminus \bigcup_k a_k) = 0$, then

$$
X = \sum_k \Big( \frac{1}{\P(a_k)} \int_{a_k} X \dd\P \Big) \ind{a_k}
\qquad \text{a.s.},
$$

and countably many numbers $\int_{a_k} X$ determine $X$ by direct read-off. This is the partition case of the first section.

*General case.* When $\P|_\cH$ has a nonatomic part there is nothing to read off, and $X$ is recovered as a limit. For bounded $X$ and $n \in \N$, let $I_{k,n} = [k 2^{-n}, (k+1)2^{-n})$, $A_{k,n} = \{X \in I_{k,n}\} \in \cH$, and $m_{k,n} = \frac{1}{\P(A_{k,n})}\int_{A_{k,n}} X$ for cells of positive probability. Every value of $X$ on $A_{k,n}$ lies in $I_{k,n}$, so $m_{k,n}$ does too, and the $\cH$-simple function $S_n = \sum_k m_{k,n} \ind{A_{k,n}}$ satisfies $\abs{X - S_n} \le 2^{-n}$ except on the null set of outcomes falling in some cell of probability zero. Hence $S_n \to X$ uniformly off a null set. For $X \in L^1$ truncate and pass to the limit.

The mechanism is this. Averaging a function over a set discards the variation of the function within the set, but on a level set $\{X \in I\}$ the variation is at most $\abs{I}$ by construction, so averages over fine level sets discard nothing in the limit. The a.s. qualifier is exactly the mass condition: on a cell of probability zero the integral is $0$ whatever $X$ is, so outcomes in such cells are invisible to the data. Averages recover values where there is mass to average against, and nowhere else.

Note that the construction used only sets in $\sigma(X) \subseteq \cH$. When $X$ is $\cH$-measurable, the sets in $\cH \setminus \sigma(X)$ contribute nothing to determining $X$; the larger $\sigma$-algebra matters only when $X$ is not $\cH$-measurable, which is when $\E[X \mid \cH] \ne X$.

:::remark{#l2-determination}
In $L^2$ this has a one-line form. $L^2(\Omega, \cH, \P)$ is the closed span of $\{\ind{A} : A \in \cH\}$, and $\int_A X = \ip{X}{\ind{A}}$. A vector in the closed span of a family is determined by its inner products with the family: if $X, X'$ share them then $X - X' \in L^2(\cH)$ is orthogonal to a total subset of $L^2(\cH)$, hence to itself, hence zero.
:::
...
In such a course this is a *definition*, adopted by analogy with the discrete formula of [[#elementary-conditional-expectation]](c), because the event $\{X = x\}$ has probability zero and the elementary definition does not apply. In this section it is a *theorem*: the formula computes the Kolmogorov conditional expectation of the previous section, in the sense of conditioning on a random variable. This is the first bridge result of the notes. An elementary formula that had to be postulated is shown to be an instance of the general definition, and the reason it is the right formula — it is a ratio of rates, the Radon–Nikodym derivative $d\mu_X/d\P_Y$ computed pointwise — becomes visible.

The scope is deliberately limited in two ways. A joint density is assumed, and only the conditional *mean* is treated: one number for each $x$. The next section removes both limitations. It constructs the entire conditional *distribution* of one random variable given another, assuming no densities, and recovers the present section as the special case in which that distribution has a density and one takes its mean. The relation between the two sections is the relation between "$\E[Y \mid X = x]$" and "the law of $Y$ given $X = x$": the former is the mean of the latter.
...
### The formula as a limit of local averages

The proof verifies the formula but does not explain it. Here is where it comes from.

Since $\E[Y \mid X]$ is $\sigma(X)$-measurable it is constant on each fiber $\{X = x\}$, and by analogy with the partition case its value there ought to be the average of $Y$ over the fiber:

$$
h(x) \;\text{``$=$''}\; \frac{1}{\P(X = x)} \int_{\{X = x\}} Y \dd\P .
$$

In the continuous case this is $0/0$: the fiber carries no mass, so it supports no average. (For the same reason [[#density-task]] cannot be proved fiber by fiber: on every fiber both integrals are $0$ and the statement is empty.) The correct version averages over a thin slab around the fiber, which does carry mass, and lets the slab shrink:

$$
h(x) = \lim_{\eps \downarrow 0}
\frac{\E\big[Y \ind{\{x \le X < x + \eps\}}\big]}{\P(x \le X < x + \eps)} .
$$
{#slab}

Each ratio is an honest conditional expectation given an event of positive probability. By the Fubini computation in the proof, the numerator equals $\int_x^{x+\eps} N(u) \dd u$ and the denominator $\int_x^{x+\eps} f_X(u) \dd u$. Both tend to zero, and the limit of their ratio is the ratio of the rates at which they do.

:::theorem{#lebesgue-differentiation title="Lebesgue Differentiation"}
If $g \in L^1_{\mathrm{loc}}(\R^n)$, then for Lebesgue-a.e. $x$ the averages of $g$ over the balls $B_\eps(x)$ converge to $g(x)$ as $\eps \downarrow 0$. In dimension one the same holds for the one-sided intervals $[x, x + \eps)$.
:::

Applying this to $N$ and to $f_X$, the limit [[#slab]] exists for a.e. $x$ with $f_X(x) > 0$ and equals
...
This is what "the average of $Y$ over the fiber $\{X = x\}$" means. The fiber has no mass, but it has a rate, and the density hypothesis is the statement that these rates exist and equal the slice integrals $N(x)$ and $f_X(x)$. The same object arises in two ways: abstractly, as the Radon–Nikodym derivative $d\mu_Y/d\P_X$, and concretely, as the limit of slab averages, which is the local reading of a density.

Note the division of labor. To *find* $h$ one divides, and the division is a limit that requires Lebesgue differentiation. To *verify* $h$ one multiplies, checking [[#cleared]], and no limit is needed. That is why the proof is short and the derivation is not.

### The elementary conditional density

Introductory courses also define, by formula, the **conditional density of $Y$ given $X = x$**:

$$
f_{Y \mid X}(y \mid x) = \frac{f(x,y)}{f_X(x)} \qquad (f_X(x) > 0) .
$$

This is a function, not yet a conditional distribution in the sense of the general theory; what it is a formula *for* is settled in the next section, where the conditional law of $Y$ given $X = x$ is defined without densities as a probability measure $\kappa(x, \cdot)$, and [[#integration-against-kernel]] shows $\kappa(x, dy) = f_{Y \mid X}(y \mid x)\, dy$ whenever a joint density exists. So the elementary conditional density is the density of the general object, and the two do not compete. The present subsection records only what can be said with the tools of this section, namely what integrating against $f_{Y \mid X}$ computes and which identities it satisfies; all of it is [[#ce-from-density]] applied to different integrands.

For each $x$ with $f_X(x) > 0$, $f_{Y \mid X}(\cdot \mid x)$ is a probability density in $y$, and [[#ce-from-density]] says $\E[Y \mid X = x] = \int y\, f_{Y \mid X}(y \mid x) \dd y$. The same argument with $Y$ replaced by $g(Y)$ or by $\ind{\{Y \in A\}}$ gives

$$
\E[g(Y) \mid X] = \int g(y)\, f_{Y \mid X}(y \mid X) \dd y,
\qquad
\P(Y \in A \mid X) = \int_A f_{Y \mid X}(y \mid X) \dd y .
$$

Three identities follow from the definition and are used constantly.

(a) *Chain rule:* $f(x,y) = f_X(x)\, f_{Y \mid X}(y \mid x)$ for a.e. $(x,y)$.

(b) *Marginalization:* $f_Y(y) = \int f_X(x)\, f_{Y \mid X}(y \mid x) \dd x$.

(c) *Bayes:* where $f_Y(y) > 0$,

$$
f_{X \mid Y}(x \mid y) = \frac{f_{Y \mid X}(y \mid x)\, f_X(x)}{f_Y(y)}
= \frac{f_{Y \mid X}(y \mid x)\, f_X(x)}
{\int f_{Y \mid X}(y \mid x')\, f_X(x') \dd x'} ,
$$

with the product in the numerator read as $0$ where $f_X(x) = 0$.

Item (c) is [[#bayes-elementary-formula]] with sums replaced by integrals: the posterior density of $X$ given the observation $Y = y$ is the prior density $f_X$ multiplied by the likelihood $f_{Y \mid X}(y \mid \cdot)$ and normalized. It is a statement about the null event $\{Y = y\}$, justified by [[#ce-from-density]] with the roles of the variables exchanged. The Bayes subsection of the next section proves the version that assumes no densities, and identifies (c) as a formula for a Radon–Nikodym derivative with respect to the prior.

## Conditional Distributions

In a first course the order of ideas is: define the conditional distribution of $X$ given $Y = y$ (a conditional mass function or conditional density), and then define the conditional expectation as its mean. Kolmogorov's theory reverses this order. Conditional *expectation* comes first: it is one Radon–Nikodym derivative, it exists on every probability space for every sub-$\sigma$-algebra, and no distribution was needed to define it. The conditional *distribution* comes second, and it is a theorem rather than a definition: it is the assertion that the conditional probabilities $\P(X \in A \mid \cH)$, one for each set $A$, can be assembled into a genuine probability measure. Assembling them requires uncountably many Radon–Nikodym derivatives to be chosen consistently, and that consistency requirement is the subject of this section.

No new notion of conditioning is introduced in this section. The basic object throughout is the conditional probability of [[#conditional-expectation]], applied to indicators:
...
an $\cH$-measurable random variable for each $A$, unique up to a null set, characterized by partial averaging: $\int_H \P(X \in A \mid \cH) \dd\P = \P(\{X \in A\} \cap H)$ for all $H \in \cH$. Every result below is either a statement of measure theory in which no conditioning occurs, or a statement about this family of random variables.

Three facts about the resulting object are established, and together they answer the question of why it is the object one should reason and act with. It is *determined*: given the probability space, the variable $X$ and the information $\cH$, the conditional distribution is fixed up to null sets, with no further choice ([[#rcd-uniqueness]]); it is not an additional modelling assumption but a consequence of the ones already made. It is *complete*: integrating a function against it gives the conditional expectation of that function, for every function at once ([[#integration-against-kernel]]), so it carries everything the theory of conditional expectation can say about $X$ given $\cH$. And it is what *decisions* reduce to: the expected loss of any decision rule is the average, over the data, of the loss integrated against the conditional distribution, so that minimizing expected loss under the model is the same as minimizing against the conditional distribution at each data value ([[#bayes-rules]]). None of these facts depends on the value of the conditional distribution at any single point, which is not determined and does not need to be.

The section is organized as follows. The first subsection states what conditional expectation provides and what is missing. The second introduces kernels, the kind of object a conditional distribution would have to be; it is pure measure theory. The next two define the regular conditional distribution, prove it unique, re-index it by the value of a conditioning variable, and prove it exists on standard Borel spaces. Then integrating against it is shown to recover every $\E[g(X) \mid \cH]$. The remaining subsections treat the density case, conditional independence, Bayes' theorem, Bayesian decision theory, and the Borel–Kolmogorov paradox.
...
*The objects of this section and how they depend on one another. Arrows point from what is used to what is built from it. The only conditioning notion is the one at the top; the regular conditional distribution is a coherent choice of versions of the conditional probabilities, and integrating against it returns to the top.*
...
These are the axioms of a probability measure, each holding almost surely. The difficulty lies in the qualifier "almost surely." Each statement holds off its own null set, and there are uncountably many statements: one for each $A$, one for each disjoint sequence. The union of uncountably many null sets need not be null, so there may be no single $\omega$ at which all the statements hold at once. Countable additivity of $A \mapsto \P(X \in A \mid \cH)(\omega)$ may fail at every $\omega$.
...
In words, a kernel is a measurable family of probability measures on $S$ indexed by $t \in T$: for each $t$ one has a distribution $\kappa(t, \cdot)$ on $S$, and the dependence on $t$ is measurable. This is the mathematical form of "the distribution of $X$ given $W = t$," of a transition probability $\P(X_{n+1} \in \cdot \mid X_n = t)$, of a statistical model $\P(\text{data} \in \cdot \mid \text{parameter} = t)$, and of a noisy channel. When densities exist, $\kappa(t, A) = \int_A f_{X \mid W}(x \mid t) \dd x$: the kernel is the conditional density integrated over $A$, and (K2) is joint measurability of $f_{X \mid W}$ in $(x, t)$. In the next subsection the index set $T$ will be $\Omega$ itself with the $\sigma$-algebra $\cH$, and later the range of a random variable.

The operation is *composition* of a measure with a kernel: from the distribution $\pi$ of $W$ and a kernel $\kappa$, produce a joint distribution on $T \times S$. In elementary terms, $p(t, x) = p(t)\,p(x \mid t)$.

:::lemma{#composition title="Composition"}
...
Let $X : \Omega \to S$ be measurable and $\cH \subseteq \cF$. A **regular conditional distribution (RCD) of $X$ given $\cH$** is a probability kernel $\kappa$ from $(\Omega, \cH)$ to $(S, \cS)$ such that for every $A \in \cS$, the random variable $\kappa(\,\cdot\,, A) : \omega \mapsto \kappa(\omega, A)$ is a version of $\P(X \in A \mid \cH)$:
...
Suppose $\cS$ is countably generated. If $\kappa, \kappa'$ are two RCDs of $X$ given $\cH$, then $\kappa(\omega, \cdot) = \kappa'(\omega, \cdot)$ for a.e. $\omega$.
:::
...
This is the first of the three facts announced at the start of the section: the conditional distribution is determined by $(\Omega, \cF, \P)$, $X$ and $\cH$ alone, and introducing it involves no choice beyond those already made in specifying the model and the information. It also inherits the characterization by $\cH$-tests set by set: for each $A$, $\kappa(\cdot, A)$ is the unique $\cH$-measurable random variable that no $\cH$-test can distinguish from $\ind{\{X \in A\}}$. It depends on $\cH$, as it must; that dependence is the tower property.
...
Let $\kappa$ be a kernel from $T$ to $S$ and put $\kappa_0(\omega, A) = \kappa(W(\omega), A)$. Then $\kappa$ is a conditional law of $X$ given $W = t$ if and only if $\kappa_0$ is an RCD of $X$ given $\sigma(W)$. Conversely, if $\cS$ is countably generated, every RCD of $X$ given $\sigma(W)$ is a.s. of the form $\kappa_0$ for some conditional law $\kappa$.
:::
...
Thus [[#conditional-law]] is [[#rcd]] with $\cH = \sigma(W)$, transported from $\Omega$ to the range of $W$, and [[#disintegration]] is partial averaging written on the range. The two forms serve different purposes. The $\sigma$-algebra form applies to arbitrary $\cH$ — a filtration, or information not generated by a single variable — and is the form in which existence is proved. The random-variable form is the one in which an observed value $t$ is substituted, and is the form taken by Bayes' theorem. Both will be used.

Three remarks on [[#disintegration]].

*It is the multiplication rule.* In the discrete case, conditioning is governed by $\P(X \in A, W = t) = \P(X \in A \mid W = t)\,\P(W = t)$. Summing over $t \in B$ gives [[#disintegration]] with a sum in place of the integral. When $\P(W = t) = 0$ for every $t$ the individual products vanish and only the integrated form survives; [[#disintegration]] is what remains of the multiplication rule. Equivalently: conditioning does not alter the joint law, it *factors* it along $W$, and composition ([[#composition]]) puts the factors back together. Disintegration and composition are inverse operations, the general forms of $p(x \mid t) = p(t,x)/p(t)$ and $p(t,x) = p(t)\,p(x \mid t)$. When densities exist, [[#disintegration]] is $f(t,x) = f_W(t)\, f_{X \mid W}(x \mid t)$ integrated over $B \times A$, with $\kappa(t, A) = \int_A f_{X \mid W}(x \mid t) \dd x$.

*It is the meaning of conditioning on a null event.* "The conditional law of $X$ given $W = t$" is defined for all $t$ at once, as a measurable rule, by the requirement that mixing the rule against the law of $W$ reproduces the joint law. No single event $\{W = t\}$ is ever conditioned on. This is the same replacement that the section on conditional expectation made for expectations: condition on $\sigma(W)$, which has meaning, rather than on $\{W = t\}$, which does not.
...
The proof handles the difficulty of the choice of versions as follows. Exceptional null sets arise only for a countable family of conditions — the rational values of the distribution function and countably many consistency conditions among them — and the uncountably many remaining sets are then handled by operations that introduce no further exceptional sets (infima, Carathéodory extension, the $\pi$–$\lambda$ theorem). This is the general method for passing from a family of a.s.-defined quantities to a pointwise-defined object, and it is where the structure of $\R$ is used: a probability measure on $\R$ is determined by countably many values of its distribution function, so only countably many versions need to be chosen.
...
:::proposition{#integration-against-kernel title="Integration Against the Conditional Law"}
...
In the setting of [[#disintegration]], $\E[g(X) \mid W = t] = \int_S g \dd\kappa(t, \cdot)$ for $\P_W$-a.e. $t$.
:::
...
Random variables are conditionally independent given $Z$ if the $\sigma$-algebras they generate are conditionally independent given $\sigma(Z)$. This is defined in terms of conditional probabilities alone; no regular conditional distribution is needed.
...
Two features of [[#bayes-risk]] should be noted before anything is derived from it. First, it is an unconditional expectation of a well-defined random variable, $L(\Theta, \delta(X))$, under a law $J$ that has been adopted explicitly; no conditioning occurs in it, no null events appear, and no posterior has been mentioned. Second, it is a functional of the *rule* $\delta$ as a whole, not of its value at any one data point. These two facts are the whole of what a Bayesian analysis commits one to: a joint law for parameter and data, and the criterion of minimal expected loss under that law. Everything else, including the role of the posterior, is a consequence.
...
Let $\Pi$ be any version of the posterior kernel.
...
Part (b) presupposes that a *measurable* pointwise minimizer $\delta^*$ exists. This is a technical condition, not a conceptual one, and it holds in the cases of interest: when $\mathcal A$ is finite or countable, a measurable minimizer can be selected by taking the smallest index achieving the minimum; under squared loss the minimizer is the posterior mean, which is measurable in $x$ by [[#integration-against-kernel]]; and measurable selection theorems cover the general case under mild conditions on $\mathcal A$ and $L$.

**What the theorem says about the posterior.** The rule "act so as to minimize posterior expected loss" is not an independent principle; it is the content of the theorem. When the one quantity the Bayesian has committed to minimizing, the expected loss [[#bayes-risk]], is written as an iterated integral by disintegrating $J$ along the data, the inner integral is the posterior expected loss, and minimizing an integrand pointwise minimizes the integral. The posterior is not a second object of belief alongside $J$; it is the Radon–Nikodym derivative through which $J$ factors along the data.
...
- With $L(\theta, a) = \abs{h(\theta) - a}$ the Bayes rule is a posterior median; with $L(\theta, a) = \ind{\{\theta \notin a\}} + c\,\nu(a)$ over sets $a$, the Bayes rules are credible sets of highest posterior density; with actions being distributions for a future observation $X'$ and a proper scoring rule as loss, the Bayes rule is the posterior predictive law $\int K(\theta, \cdot)\, \Pi(x, d\theta)$.
...
**The posterior at the realized data.** Two versions of the posterior differ on an $m$-null set of data values, and the observed data $x_0$ is a single point, so for any $x_0$ there is a version that has been modified there. The framework assigns no meaning to $\Pi(x_0, \cdot)$ in isolation.

[[#bayes-rules]] makes this harmless without appeal to any property of the point $x_0$. The action taken at $x_0$ is justified not by a belief about the posterior at $x_0$ but as the output, at the observed data, of a rule $\delta^*$ with minimal expected loss under the model. Two Bayes rules may prescribe different actions at $x_0$, but only if $x_0$ lies in an $m$-null set on which they differ, where they have the same Bayes risk. Expected loss under $J$ cannot distinguish behavior on an $m$-null set of data values; that is what $m$-null means. To ask for a uniquely correct action at exactly $x_0$ is to ask for something finer than the only criterion in force can express. The indifference between Bayes rules on null sets is a consequence of the criterion, as the a.e.-uniqueness of $\E[X \mid Y = y]$ is a consequence of [[#almost-sure-identification]].
...
This completes the three facts announced at the start of the section. The conditional distribution is determined by the model and the information ([[#rcd-uniqueness]]), it determines every conditional expectation ([[#integration-against-kernel]]), and expected loss under the model is minimized by minimizing against it ([[#bayes-rules]]). Each statement concerns the conditional distribution as a function of the data; none requires its value at a single point, and none of its uses does either.

:::remark{#canonical-version title="Selecting a Canonical Version"}
If one wants a distinguished version of the posterior — a way of resolving the indifference on null sets — one must add a principle outside expected loss. The usual one is regularity of the model in the data: if $\nu$ is Lebesgue measure and $x \mapsto p(\theta, x)$ is continuous with suitable domination, then the Bayes formula defines a version continuous in $x$, any two continuous versions agree at every $x$ in the support of $m$, and at every such $x$ the value is the limit of the elementary posteriors $\P(\Theta \in \cdot \mid X \in B_\eps(x))$ as $\eps \downarrow 0$. This selects a canonical version and gives its point values an interpretation, but it is a convention added to the theory; the justification of Bayes rules in [[#bayes-rules]] does not use it.
:::

### The Borel–Kolmogorov paradox

The a.e.-uniqueness of conditional distributions has a consequence that is easy to state and easy to get wrong: a null event does not have a conditional distribution. Only a random variable does, and two random variables that share a null fiber can assign different conditional distributions to it. The classical illustration is a point chosen uniformly on a sphere.
...
**The example.** Let $P$ be a point chosen uniformly on the unit sphere: $\P(P \in R) = \sigma(R)/4\pi$ for Borel $R \subseteq S^2$, where $\sigma$ is surface area. This law is invariant under rotations. In the coordinates of the figure, surface area is $\sigma(dA) = \cos\phi\, d\phi\, d\lambda$ — a longitude interval $d\lambda$ spans an arc of length $\cos\phi\, d\lambda$ at latitude $\phi$, since meridians converge toward the poles — so the latitude and longitude $(\Phi, \Lambda)$ of $P$ have joint density
...
The equator $E = \{\Phi = 0\}$ is a great circle. So is the meridian at $\lambda_0$ together with the meridian opposite it, the set $M = \{\Lambda \in \{\lambda_0, \lambda_0 + \pi\}\}$, which is a fiber of the random variable $\Lambda \bmod \pi$. A rotation $\rho$ carries $M$ onto $E$ and preserves the law of $P$, so it seems that "the conditional distribution of $P$ given that $P$ lies on a great circle" should be the same for both, up to rotation. It is not: uniform on $E$, $\cos\phi$-weighted on $M$. That is the paradox.

To state the conflict on a single circle, let $\Lambda' = (\Lambda \bmod \pi) \circ \rho^{-1}$ be longitude measured with respect to the rotated axis, so that $E$ is a fiber of $\Lambda'$ as well as of $\Phi$. Since $\rho(P)$ has the same law as $P$, the conditional law of $P$ given $\Lambda'$ at the fiber $E$ is the $\rho$-image of the $\cos\phi$-weighted law on $M$: a non-uniform distribution on $E$. The conditional law of $P$ given $\Phi$ at the same fiber $E$ is uniform. Two random variables have $E$ as a common fiber, and their conditional laws there differ.
...
**Why the two answers differ.** Each conditional law is what elementary conditioning on a positive probability event becomes as the event shrinks to the circle, and the two events are the band and the wedge of the figure. Here the elementary computation is exact. For an interval $I$ of longitudes,
...
*"The distribution given that $P$ lies on the equator" is not well-defined, for two separate reasons.* First, it does not say which random variable placed $P$ there: $\Phi$ and $\Lambda'$ give different laws on the same circle, and no convention removes this. Second, even once $\Phi$ is fixed, the conditional law $\kappa_\Phi(\phi, \cdot)$ is determined only for $\P_\Phi$-a.e. $\phi$, and $\{0\}$ is a $\P_\Phi$-null set of values; so "given $\Phi = 0$" is the value of an a.e.-defined rule at a single point, which a different version may alter. This is the same situation as the posterior at the observed data. What is well-defined is the rule $\phi \mapsto \kappa_\Phi(\phi, \cdot)$; the statement "uniform on the equator" is that rule evaluated at $0$ for the version given by the density formula, with both the random variable and the version understood.

*This does not make conditional distributions arbitrary.* Given the $\sigma$-algebra, the conditional law is determined ([[#rcd-uniqueness]]), and every quantity computed or decided with it is invariant under versions ([[#bayes-rules]]). The indeterminacy is confined to two places: the specification of what one conditions on, which is a modelling input that the user must supply, and null sets of values, which no criterion in the framework can see. The paradox is not the theory failing to answer a question; it is a question that was never completely asked.

## Summary

- Conditioning on an event of positive probability is forced by two requirements: the event becomes certain and relative weights inside it do not change. The result $\P(A \mid B) = \P(A \cap B)/\P(B)$ is $\P$ reweighted by the density $\ind{B}/\P(B)$.
- The Radon–Nikodym theorem characterizes reweightings of a measure: $\nu \ll \mu$ iff $\nu = f\,d\mu$ for a density $f$, unique a.e., which is both the unique function with the right set-integrals and the a.e. limit of ratios of masses of shrinking sets. Likelihood ratios and changes of measure are instances.
- $\E[X \mid \cH]$ is the density of $(X\,d\P)|_\cH$ with respect to $\P|_\cH$: the unique $\cH$-measurable random variable agreeing with $X$ in every average an observer with information $\cH$ can compute. In $L^2$ it is the orthogonal projection onto $L^2(\cH)$ and the best mean-square predictor; the regression function of statistics is this object.
- When $(X, Y)$ has a joint density, $\E[Y \mid X = x] = \int y\, f(x,y) \dd y / f_X(x)$ is the ratio of the rates at which $Y$-mass and probability mass accumulate across the null fiber $\{X = x\}$. Verifying the formula clears the denominator; deriving it takes a limit of slab averages.
- The conditional *distribution* is not a new primitive notion but a theorem about conditional probabilities: the random variables $\P(X \in A \mid \cH) = \E[\ind{\{X \in A\}} \mid \cH]$, one a.s.-class per set $A$, can be realized as a single kernel — a measurable family of probability measures — when $X$ takes values in a standard Borel space, uniquely a.e. Integrating against it recovers every $\E[g(X) \mid \cH]$, so it is a complete summary. Re-indexed by the value of a conditioning variable it is the conditional law given $W = t$, characterized by the disintegration identity, which is the multiplication rule in integrated form. Bayes' theorem disintegrates a joint law built from prior and likelihood the other way round; under a dominating measure the posterior is the prior reweighted by the likelihood and normalized, and it is the conditional law of $\Theta$ given $\sigma(X)$. The Bayesian commitment is to the joint law of parameter and data and to minimal expected loss under it; the rule "minimize posterior expected loss" is a theorem, obtained by disintegrating the joint expected loss along the data, and the indeterminacy of the posterior on null sets of data values is invisible to that criterion.
